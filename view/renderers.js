...
export function renderModelBadge(modelName) {
    const container = elements.modelBadgeContainer;
    if (!container) return;
    ...
    container.innerHTML = `
        <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 shadow-sm transition-all hover:bg-indigo-100/70">
            <span class="relative flex h-2 w-2">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                <span class="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span>
            </span>
            <span>核心大腦：${friendlyName}</span>
        </div>
    `;
...
export function renderQuizSummary(summaryData) {
    ...
    // 渲染模型徽章
    if (summaryData.modelName) {
        renderModelBadge(summaryData.modelName);
    }
