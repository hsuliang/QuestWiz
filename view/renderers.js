...
function createQuestionElement(q, index, callbacks = {}) {
    ...
    if (bloomBadge && q.bloomLevel) {
        ...
        bloomBadge.textContent = bloomNames[levelKey] || q.bloomLevel;
...
export function renderQuizSummary(summaryData) {
    ...
    summaryContainer.innerHTML = `
        <div class="bg-indigo-50/50 border border-indigo-100 rounded-xl p-4 shadow-sm mb-6">
            ...
            <div class="flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-indigo-700 font-medium">${legendHtml}</div>
        </div>
    `;
}
...
export function renderLibraryQuizzes(quizzes, onImport, onDelete) {
    ...
    wrapper.className = "max-h-[600px] overflow-y-auto overflow-x-auto border border-gray-200 rounded-xl bg-white shadow-sm custom-scrollbar";
    ...
    quizzes.forEach((quiz, index) => {
        ...
        tr.className = "group hover:bg-indigo-50/50 transition-colors border-b last:border-b-0 border-gray-100";
        ...
        importBtn.className = "inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-sm transition-all active:scale-95";
