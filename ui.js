...
export function renderQuestionsForEditing(questions) {
    ...
    questions.forEach((q, index) => {
        ...
        // --- [新增] 資料正規化邏輯 ---
        let rawCorrect = q.correct;
        // 如果是數字或字串，轉成陣列
        if (typeof rawCorrect === 'number' || typeof rawCorrect === 'string') {
            rawCorrect = [parseInt(rawCorrect, 10)];
        } else if (!Array.isArray(rawCorrect)) {
            // 如果是 null 或 undefined，且是非題，根據 is_correct 判斷
            if (isTF) rawCorrect = [q.is_correct ? 0 : 1];
            else rawCorrect = []; // 預設空陣列
        }

        const questionData = isTF ? { text: q.text, options: ['是', '否'], correct: rawCorrect, time: q.time || 30, explanation: q.explanation || '', design_concept: q.design_concept || '' } : {...q, correct: rawCorrect};
